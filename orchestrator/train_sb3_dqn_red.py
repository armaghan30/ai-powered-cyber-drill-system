...
def main():
    ...
    model = DQN(
        ...
        verbose=1,
        tensorboard_log="./tb_logs/sb3_dqn_red",
    )

    callback = RewardLoggerCallback()

    print(f"[SB3 DQN RED] Training for {total_timesteps} timesteps on {topology_path}...")
    model.learn(total_timesteps=total_timesteps, callback=callback)

    model_path = f"results/models/sb3_dqn_red_{topo}"
    csv_path = f"results/csv/sb3_dqn_red_{topo}.csv"

    model.save(model_path)
    print(f"[SB3 DQN RED] Model saved -> {model_path}.zip")
    ...
    print(f"[SB3 DQN RED] {len(callback.episode_rewards)} episodes logged -> {csv_path}")
