...
"""
Evaluate trained RED and BLUE DQN agents together
on the same orchestrator environment.
"""
...
def decode_red_action(index: int, host_order: List[str], orch: Orchestrator) -> Dict[str, Any]:
    """
    Map RED action index -> concrete action dict, using the same RedAgent
    instance that orchestrator uses so its knowledge (known_vulns) stays
    consistent with scans/exploits.
    ...
    if index < n:
        ...
        # This updates orch.red_agent.known_vulns and prints scan logs
        return orch.red_agent.scan(host)
    elif index < 2 * n:
        host = host_order[index - n]
        # This sets 'success' and prints exploit logs
...
def decode_blue_action(index: int, host_order: List[str]) -> Dict[str, Any]:
    """
    Map BLUE action index -> environment action dict.
...
def main():
    ...
    # Load trained RL agents (already standardized)
